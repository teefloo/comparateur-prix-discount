...
import { extractGifiCategoryPageOffers, extractGifiProductPageOffer, parseGifiPrice } from '../../src/lib/scrapers/gifi-scraper'
  ...
  <div class="product-tile position-relative">
    <div class="image-container">
      <a href="/maison/rangement-et-entretien/nettoyage-et-entretien/eponge-brosse-et-chiffon/bobine-essuie-tout-tenerella-xl-x2/000000000000405281.html">
        <img class="tile-image gtm-button" src="/dw/image/v2/BJSK_PRD/on/demandware.static/-/Sites-master_GIFI/default/dw8a09a463/4/0/405281_F.jpg?sw=156&amp;sh=156&amp;sm=fit" alt="Bobine essuie tout Tenerella XL x2" />
      </a>
    </div>
    <div class="tile-body">
          ...
          <span class="sales fs-lg-16 lh-20">
            <span class="value">
              <div class="formatted-price align-items-start formatted-price--pseudo-1">
                <span class="sr-only">4,99 €</span>
                <span class="formatted-price__before-decimal" data-integer="4"></span>
                <span class="formatted-price__after-decimal">
                  <span class="formatted-price__currency" data-currency-symbol="€" data-currency="EUR"></span>
    ...
    <script type="application/ld+json">
      {
        ...
        "image": ["https://www.gifi.fr/dw/image/v2/BJSK_PRD/on/demandware.static/-/Sites-master_GIFI/default/dwa4af3a3f/6/2/625527_F.jpg?sw=634&sh=634&sm=fit"],
...
test('parses GIFI category tiles with normalized absolute urls and prices', () => {
  const offers = extractGifiCategoryPageOffers(categoryTileHtml, 'https://www.gifi.fr/maison/rangement-et-entretien/nettoyage-et-entretien/')

  assert.equal(offers.length, 1)
  assert.equal(offers[0]?.retailer, 'gifi')
  assert.equal(offers[0]?.sourceProductId, '000000000000405281')
  assert.equal(offers[0]?.sourceUrl, 'https://www.gifi.fr/maison/rangement-et-entretien/nettoyage-et-entretien/eponge-brosse-et-chiffon/bobine-essuie-tout-tenerella-xl-x2/000000000000405281.html')
  assert.equal(offers[0]?.image, 'https://www.gifi.fr/dw/image/v2/BJSK_PRD/on/demandware.static/-/Sites-master_GIFI/default/dw8a09a463/4/0/405281_F.jpg?sw=156&sh=156&sm=fit')
...
test('parses GIFI product pages from json-ld and gtm metadata', () => {
  const offer = extractGifiProductPageOffer(productPageHtml, 'https://www.gifi.fr/maison/rangement-et-entretien/nettoyage-et-entretien/produits-d-entretien/spray-nettoyant-multi-usage-cerise-750-ml/000000000000625527.html')

  assert.ok(offer)
  assert.equal(offer?.retailer, 'gifi')
  assert.equal(offer?.sourceProductId, '000000000000625527')
  assert.equal(offer?.price, 1.55)
  assert.equal(offer?.image, 'https://www.gifi.fr/dw/image/v2/BJSK_PRD/on/demandware.static/-/Sites-master_GIFI/default/dwa4af3a3f/6/2/625527_F.jpg?sw=634&sh=634&sm=fit')
...
test('parses GIFI price strings', () => {
  assert.equal(parseGifiPrice('4,99 €'), 4.99)
  assert.equal(parseGifiPrice('1.55 €'), 1.55)
})
