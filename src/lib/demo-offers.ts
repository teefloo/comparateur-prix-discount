...
import { applyPriceFilters, type PriceSortOption } from './result-filters'
import { filterOffersByQuery, normalizeRetailerSelection, sortRetailerOfferCards } from './scraper-utils'
import type { RetailerOfferCard } from './types'

export const DEMO_OFFERS: RetailerOfferCard[] = [
  {
...
export function filterDemoOffers(options: {
  query?: string | null
  ...
  const selectedRetailers = normalizeRetailerSelection(options.retailer)
  let offers = DEMO_OFFERS

  if (options.category) {
    offers = offers.filter((offer) => offer.category === options.category)
  }
  ...
  if (options.query) {
    offers = filterOffersByQuery(offers, options.query)
  }
  ...
  if (options.sort === 'default' || !options.sort) {
    offers = sortRetailerOfferCards(offers)
  }
